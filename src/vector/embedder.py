...
# Modular ingestion steps

def chunk_scope(scope: str):
    paths = get_scope_paths(scope)
    os.makedirs(paths["bronze"], exist_ok=True)
    ...
    chunks = split_documents(raw_docs)
    save_chunks_to_silver(chunks, paths["silver"])
    print(f"Chunking complete: {len(chunks)} chunks saved to Silver.")
    return chunks


def embed_scope(scope: str, chunks: list = None):
    paths = get_scope_paths(scope)
    os.makedirs(paths["gold"], exist_ok=True)

    if chunks is None:
        chunks = load_scope_chunks(scope)

    store_embeddings(chunks, paths["gold"])
    print(f"Embedding complete: FAISS index stored in Gold for scope '{scope}'.")


def build_graph(scope: str, chunks: list = None):
    if chunks is None:
        chunks = load_scope_chunks(scope)
    ...
    graph_builder.process_chunks(chunks, scope)
    graph_builder.close()
    print(f"Graph build complete for scope '{scope}'.")
